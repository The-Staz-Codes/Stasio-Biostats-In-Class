...
```{r}
#View what the names are in the columns of the dataset
names(rabies)

#Fix the names by changing dots to underscores
names(rabies) <- c("Received_Year", "Received_Month", "Report_Date",          
"County_Name", "Animal_Classification", "Number_of_Samples",    
"Positive_Samples")
```
...
levles(rabies$Animal_Classification)
```
